...
|06.|**Stream-preparation utilities**:<br/>Ready-made functions for a source's `transform` option — they get a raw feed ready for analytics: fix numeric types, normalize timestamps, keep a replay window, label shifts, track activity, stamp period keys. Promoted from field use: `coerceNumeric`, `normalizeTimestamp`, `filterRows`, `labelShift`, `trackActivity`, `stampPeriod`. Built with full tests and a [handbook page](docs/handbook/stream-preparation.md); ships with the next release.|Medium|In progress|
