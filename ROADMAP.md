...
|05.|**Handbook audit**:<br/>Verify every handbook page against the released code — options against their schemas, every snippet run. Repeats each release. A first correctness pass rode 0.6.0: quoted log lines re-verified against the shipped grammar. A second pass rode 0.7.0: the eight pages that release touched were checked claim by claim against the code, with 21 corrections. The full options-and-snippets audit remains.|Medium|In progress|
...
|09.|**Soak program on edge hardware**:<br/>Multi-day continuous runs on an industrial Raspberry Pi, a Revolution Pi Connect 5 (Compute Module 5, 8 GB RAM, 32 GB eMMC), with exact message accounting. The first multi-day run found a silent write loss that 0.7.0 fixes. The fix then ran four days on the board at twice the plant rate, with exact accounting and no loss. 0.8.0 is that code with the announced removals, and it is the 1.0.0 candidate. The results back the 1.0.0 stability declaration.|High|In progress|
