...
|09.|**Soak program on edge hardware**:<br/>Multi-day continuous runs on industrial Raspberry-Pi-class hardware with exact message accounting. The first multi-day run found a silent write loss that 0.7.0 fixes, and the fix then ran 60 hours on the board with exact accounting. The full run repeats on 0.7.0. The results back the 1.0.0 stability declaration.|High|In progress|
